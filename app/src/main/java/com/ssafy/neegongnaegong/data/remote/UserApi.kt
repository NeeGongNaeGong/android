...
interface UserApi {
    @GET("/api/users/{id}")
    suspend fun getUser(@Path("id") id: Long): Response<ApiResponse<UserDetailResponse>>
    ...
    ): Response<ApiResponse<PageableData<UserDetailResponse>>>

    @GET("/api/users/nickname/validate")
    suspend fun validateNickname(@Query("nickname") nickname: String): Response<ApiResponse<ValidateNicknameResponse>>

    @POST("/api/users")
    suspend fun updateUser(@Body request: UpdateUserRequest): Response<ApiResponse<Unit>>
}
