...
class NetworkNotificationDataSourceImpl
    @Inject
    constructor(
        ...
        override fun getNotification(notificationId: Long): Flow<GetNotificationResponse> =
            apiFlow {
        ...
        override fun readNotification(notificationId: Long): Flow<Unit> =
            apiFlow {
                ...
                if (throwable is ApiException.ConflictException) {
                    emit(value = Unit)
                } else {
                    throw throwable
                }
